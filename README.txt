...
------------------------------------------------------
The app tries TWO scanning engines automatically, in this order,
and just uses whichever one is available — you don't need to pick:

  1. NATIVE scanning (fastest, built into the browser, zero
     download). Available today on Chrome/Edge on ANDROID and on
     macOS. If this is available you'll see no extra message under
     the Start Scanning button.

  2. ZXING fallback (free, open-source, MIT-licensed JavaScript
     barcode reader — https://github.com/zxing-js/browser). This
     kicks in automatically on platforms where the browser has no
     native scanner — most notably WINDOWS and LINUX desktop
     Chrome/Edge, which never got native support even in the
     newest browser versions, because it depends on a feature the
     operating system itself has to provide. If this is what's
     active, you'll see "Using the built-in fallback scanner for
     this browser/device." under the Start Scanning button. It's
     slightly slower than native but works the same way from your
     point of view: point the camera, get a beep/vibration, confirm
     the item.

  3. If NEITHER is available (for example you're offline the very
     first time you open the app, so the fallback library couldn't
     download, or you're on a browser with no camera support at
     all such as some in-app browsers), the Start Scanning button
     is greyed out and disabled, and you'll see a message saying
     so. "+ Add item manually" always works regardless, on any
     device or browser.

Practical notes:
  - iPhone Safari: the ZXing fallback still works there too (it's
    pure JavaScript, not tied to one browser), so scanning should
    work on iPhone as well as Android and desktop.
  - The app needs an internet connection the FIRST time you open it
    on a given device, to download the ZXing fallback library and
    the Excel export tool (both load from free public CDNs). After
    that, the service worker (sw.js) caches them for offline reuse.
  - Product name auto-fill (via Open Food Facts) always needs an
    internet connection — if you're offline, just type the item
    name in yourself; everything else keeps working.
